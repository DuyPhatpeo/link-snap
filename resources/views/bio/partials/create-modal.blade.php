...
<div id="createBioModal" class="fixed inset-0 z-[100] hidden overflow-y-auto bg-slate-950/60 backdrop-blur-md">
    <div onclick="if(event.target===this) BioManager.closeCreateModal()" class="flex min-h-full items-center justify-center p-4">
        <div class="relative w-full max-w-lg bg-white/95 backdrop-blur-2xl rounded-3xl sm:rounded-[36px] shadow-2xl p-6 sm:p-9 border border-white/90 animate-in zoom-in-95 duration-200">
            <button onclick="BioManager.closeCreateModal()" class="absolute top-5 right-5 text-slate-400 hover:text-slate-900 transition-colors p-1" aria-label="Đóng">
            ...
            <form id="createBioForm" onsubmit="BioManager.handleCreate(event)" class="space-y-3.5">
                ...
                <div>
                    ...
                    <input type="text" name="title" placeholder="VD: Duy Phát | Creator" required
                        class="w-full bg-slate-50/80 border border-slate-200 rounded-xl sm:rounded-2xl py-2.5 px-3.5 text-xs font-semibold text-slate-800 outline-none focus:border-indigo-500 focus:bg-white focus:ring-4 focus:ring-indigo-50 transition-all">
                </div>
                ...
                <div>
                    ...
                    <div class="flex items-center bg-slate-50/80 border border-slate-200 rounded-xl sm:rounded-2xl overflow-hidden focus-within:border-indigo-500 focus-within:bg-white focus-within:ring-4 focus-within:ring-indigo-50 transition-all">
                ...
                <div>
                    ...
                    <textarea name="bio" placeholder="Giới thiệu đôi nét về bản thân hoặc dự án..." rows="3"
                        class="w-full bg-slate-50/80 border border-slate-200 rounded-xl sm:rounded-2xl py-2.5 px-3.5 text-xs font-semibold text-slate-800 outline-none focus:border-indigo-500 focus:bg-white focus:ring-4 focus:ring-indigo-50 transition-all resize-none"></textarea>
                </div>
