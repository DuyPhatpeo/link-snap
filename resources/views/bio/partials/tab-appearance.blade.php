...
<div id="tab-appearance" class="tab-content hidden animate-in fade-in slide-in-from-bottom-4 duration-300">
    <section class="glass-card rounded-2xl p-6 sm:p-8 border border-slate-200/80 shadow-sm">
        <div class="mb-8">
        ...
        <form onsubmit="Editor.saveInfo(event)" class="space-y-8">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" name="title" value="{{ $bioPage->title }}" required
                                ...
                                class="w-full bg-slate-50 border border-slate-200 rounded-xl py-2.5 px-3.5 text-slate-800 font-bold text-xs focus:bg-white focus:border-indigo-500 outline-none transition-all">
                        </div>
                        <div>
                            ...
                            <textarea name="bio" rows="3"
                                ...
                                class="w-full bg-slate-50 border border-slate-200 rounded-xl py-2.5 px-3.5 text-slate-800 font-bold text-xs focus:bg-white focus:border-indigo-500 outline-none transition-all resize-none">{{ $bioPage->bio }}</textarea>
                        </div>
                    </div>
                        ...
                        <div class="relative group">
                            ...
                            <div class="relative">
                                <input type="url" name="profile_image" id="profileImageInput" value="{{ $bioPage->profile_image }}" placeholder="https://..."
                                    ...
                                    class="w-full bg-slate-50 border border-slate-200 rounded-xl py-2.5 pl-3.5 pr-12 text-slate-800 font-bold text-xs focus:bg-white focus:border-indigo-500 outline-none transition-all">
                                <div class="absolute right-3 top-1/2 -translate-y-1/2 w-7 h-7 rounded-lg overflow-hidden border border-slate-200 shadow-sm">
                                    <img id="profileImagePreview" src="{{ $bioPage->profile_image ?? asset('logo.png') }}" class="w-full h-full object-cover">
                                </div>
                            </div>
                        </div>
                        <div class="p-3.5 bg-indigo-50/60 rounded-xl border border-indigo-100/60">
            ...
            <div class="space-y-5 pt-2">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div>
                        ...
                        <div class="flex p-1 bg-slate-50 rounded-xl border border-slate-200">
                            <button type="button" onclick="Editor.setBioAlign('left')" id="align-btn-left"
                                class="bio-align-btn flex-1 py-1.5 rounded-lg transition-all flex items-center justify-center {{ ($theme['bio_text_align'] ?? 'center') === 'left' ? 'bg-white shadow-sm text-slate-900' : 'text-slate-400 hover:text-slate-600' }}">
                            ...
                            <button type="button" onclick="Editor.setBioAlign('center')" id="align-btn-center"
                                class="bio-align-btn flex-1 py-1.5 rounded-lg transition-all flex items-center justify-center {{ ($theme['bio_text_align'] ?? 'center') === 'center' ? 'bg-white shadow-sm text-slate-900' : 'text-slate-400 hover:text-slate-600' }}">
                            ...
                            <button type="button" onclick="Editor.setBioAlign('right')" id="align-btn-right"
                                class="bio-align-btn flex-1 py-1.5 rounded-lg transition-all flex items-center justify-center {{ ($theme['bio_text_align'] ?? 'right') === 'right' ? 'bg-white shadow-sm text-slate-900' : 'text-slate-400 hover:text-slate-600' }}">
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            <select name="theme_data[bio_text_size]" 
                                ...
                                class="w-full bg-slate-50 border border-slate-200 py-2 px-2.5 rounded-xl text-xs font-bold text-slate-700 outline-none focus:bg-white focus:border-indigo-500 transition-all">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-4">
                        <div>
                            ...
                            <div class="flex p-1 bg-slate-50 rounded-xl border border-slate-200">
                                <button type="button" onclick="Editor.setButtonType('solid')" id="type-btn-solid"
                                    class="btn-type-btn flex-1 py-1.5 rounded-lg text-[10px] font-bold uppercase transition-all {{ ($theme['button_type'] ?? 'solid') === 'solid' ? 'bg-white shadow-sm text-slate-900' : 'text-slate-400 hover:text-slate-600' }}">
                                    Đầy
                                </button>
                                <button type="button" onclick="Editor.setButtonType('outline')" id="type-btn-outline"
                                    class="btn-type-btn flex-1 py-1.5 rounded-lg text-[10px] font-bold uppercase transition-all {{ ($theme['button_type'] ?? '') === 'outline' ? 'bg-white shadow-sm text-slate-900' : 'text-slate-400 hover:text-slate-600' }}">
                                    Viền
                                </button>
                                <button type="button" onclick="Editor.setButtonType('soft')" id="type-btn-soft"
                                    class="btn-type-btn flex-1 py-1.5 rounded-lg text-[10px] font-bold uppercase transition-all {{ ($theme['button_type'] ?? '') === 'soft' ? 'bg-white shadow-sm text-slate-900' : 'text-slate-400 hover:text-slate-600' }}">
                                    Mờ
                                </button>
                            </div>
                        ...
                        <div>
                            ...
                            <div class="flex p-1 bg-slate-50 rounded-xl border border-slate-200 overflow-x-auto no-scrollbar">
                                <button type="button" onclick="Editor.setButtonStyle('none')" id="style-btn-none"
                                    class="btn-style-btn flex-1 min-w-[50px] py-1.5 rounded-lg text-[10px] font-bold uppercase transition-all {{ ($theme['button_style'] ?? '2xl') === 'none' ? 'bg-white shadow-sm text-slate-900' : 'text-slate-400 hover:text-slate-600' }}">
                                    Vuông
                                </button>
                                <button type="button" onclick="Editor.setButtonStyle('xl')" id="style-btn-xl"
                                    class="btn-style-btn flex-1 min-w-[50px] py-1.5 rounded-lg text-[10px] font-bold uppercase transition-all {{ ($theme['button_style'] ?? '2xl') === 'xl' ? 'bg-white shadow-sm text-slate-900' : 'text-slate-400 hover:text-slate-600' }}">
                                    Vừa
                                </button>
                                <button type="button" onclick="Editor.setButtonStyle('2xl')" id="style-btn-2xl"
                                    class="btn-style-btn flex-1 min-w-[50px] py-1.5 rounded-lg text-[10px] font-bold uppercase transition-all {{ ($theme['button_style'] ?? '2xl') === '2xl' ? 'bg-white shadow-sm text-slate-900' : 'text-slate-400 hover:text-slate-600' }}">
                                    Lớn
                                </button>
                                <button type="button" onclick="Editor.setButtonStyle('full')" id="style-btn-full"
                                    class="btn-style-btn flex-1 min-w-[50px] py-1.5 rounded-lg text-[10px] font-bold uppercase transition-all {{ ($theme['button_style'] ?? '2xl') === 'full' ? 'bg-white shadow-sm text-slate-900' : 'text-slate-400 hover:text-slate-600' }}">
                                    Tròn
                                </button>
                            </div>
            ...
            <div class="pt-4 border-t border-slate-100 flex justify-end">
                <button type="submit" class="bg-indigo-600 hover:bg-indigo-700 text-white font-bold font-outfit px-6 py-2.5 rounded-xl shadow-sm transition-all active:scale-95 text-xs uppercase tracking-wider">
