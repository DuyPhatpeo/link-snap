...
<div id="tab-links" class="tab-content transition-all duration-300">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        ...
        <button onclick="Editor.openLinkModal()" class="w-full sm:w-auto flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white font-bold font-outfit px-5 py-2.5 rounded-xl shadow-sm transition-all active:scale-95 text-xs uppercase tracking-wider">
    ...
    <div class="mb-6">
        ...
        <div id="socialIconsList" class="space-y-2.5">
            ...
            <div data-sort-id="{{ $link->id }}" class="group flex items-center gap-3.5 bg-white p-3.5 sm:p-4 rounded-xl border border-slate-200/80 shadow-sm hover:shadow-md hover:border-indigo-200 transition-all">
                ...
                <div class="w-9 h-9 flex items-center justify-center bg-indigo-50 text-indigo-600 rounded-lg shrink-0">
                    @if($link->icon)
                ...
                <div class="flex items-center gap-1 shrink-0">
                    <button data-id="{{ $link->id }}" data-label="{{ $link->label }}" data-url="{{ $link->url }}" data-type="{{ $link->type }}" data-icon="{{ $link->icon }}"
                        ...
                        class="p-2 text-slate-400 hover:text-indigo-600 bg-slate-50 hover:bg-indigo-50 rounded-lg transition-all" title="Chỉnh sửa">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                    </button>
                    <button onclick="Editor.deleteLink('{{ $link->id }}')"
                        class="p-2 text-slate-400 hover:text-rose-500 bg-slate-50 hover:bg-rose-50 rounded-lg transition-all" title="Xóa">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                    </button>
                </div>
            </div>
            @empty
            <div class="bg-slate-50/70 rounded-xl p-5 text-center border border-dashed border-slate-200">
    ...
    <div>
        <div class="flex items-center gap-3 mb-3">
            <div class="flex items-center gap-2">
                <div class="w-2 h-2 rounded-sm bg-violet-500"></div>
        ...
        <div id="buttonLinksList" class="space-y-2.5">
            ...
            <div data-sort-id="{{ $link->id }}" class="group flex items-center gap-3.5 bg-white p-3.5 sm:p-4 rounded-xl border border-slate-200/80 shadow-sm hover:shadow-md hover:border-violet-200 transition-all">
                <div class="drag-handle cursor-move text-slate-300 group-hover:text-slate-500 transition-colors p-1 -ml-1 shrink-0" title="Kéo để đổi vị trí">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M4 8h16M4 16h16" /></svg>
                </div>
                <div class="w-9 h-9 flex items-center justify-center bg-violet-50 text-violet-600 rounded-lg shrink-0">
