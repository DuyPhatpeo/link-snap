...
<head>
    ...
    <style>
        :root {
            --bg-color: {{ $theme['background'] ?? '#f0f2f8' }};
            --title-color: {{ $theme['text_color'] ?? '#0f172a' }};
            --text-color: {{ $theme['text_color'] ?? '#334155' }};
            --bio-text-color: {{ $theme['bio_text_color'] ?? '#64748b' }};
            --bio-bg-color: {{ $theme['bio_bg_color'] ?? 'transparent' }};
            --bio-align: {{ $theme['bio_text_align'] ?? 'center' }};
            --btn-bg: {{ $theme['button_bg'] ?? '#2563eb' }};
        ...
        * { box-sizing: border-box; }
        ...
        .bio-btn-soft {
            background-color: color-mix(in srgb, var(--btn-bg), transparent 90%);
            color: var(--btn-bg);
        }
        .bio-btn-glass {
...
<body class="flex flex-col min-h-screen {{ (!isset($theme['background']) || $theme['background'] === '#f0f2f8' || $theme['background'] === '#f8fafc') ? 'default-bg' : '' }}">
    <div class="max-w-[420px] w-full mx-auto px-5 py-14 flex flex-col flex-1">
        ...
        <div class="flex flex-col items-center text-center mb-8">
            ...
            <h1 id="preview-title" class="bio-title text-[22px] font-black tracking-tight" style="letter-spacing: -0.02em; color: var(--title-color);">{{ $bioPage->title }}</h1>
        ...
        @if($bioPage->bio)
        @php
            $hasBioBg = isset($theme['bio_bg_color']) && 
                        $theme['bio_bg_color'] !== 'transparent' && 
        ...
        <div id="preview-button-links" class="space-y-3 w-full">
            ...
            @php
                ...
                $isDefaultBtn = !isset($theme['button_bg']);
                
                $btnClass = "bio-btn bio-link-btn flex items-center justify-center w-full py-4 px-6 rounded-$btnRadius text-center relative group active:scale-[0.98]";
                if ($isDefaultBtn) {
                    $btnClass .= " bio-btn-glass";
                } else {
                    $btnClass .= " bio-btn-$btnType";
                }
            @endphp
    ...
    {{-- Simplified Platform Footer --}}
    <footer class="w-full max-w-[420px] mx-auto px-5 pb-10 mt-auto">
        <div class="flex flex-col items-center gap-3 opacity-60 hover:opacity-100 transition-opacity duration-500">
            <a href="/" class="flex items-center gap-2 group">
                <div class="p-1.5 bg-white rounded-lg shadow-sm border border-slate-100 group-hover:scale-110 transition-transform">
                    <img src="{{ asset('logo.png') }}" alt="LinkSnap" class="w-4 h-4">
                </div>
                <span class="text-sm font-black text-slate-800 tracking-tighter italic">LinkSnap</span>
            </a>
            <p class="text-[9px] font-bold text-slate-400 uppercase tracking-[0.2em]">Tạo trang Bio miễn phí của bạn</p>
        </div>
    </footer>
</body>
</html>