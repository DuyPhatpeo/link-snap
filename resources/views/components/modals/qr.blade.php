...
<div id="qrModal" onclick="if(event.target===this) Modal.close('qrModal')" class="fixed inset-0 bg-slate-950/60 backdrop-blur-md z-50 flex items-center justify-center p-4 hidden animate-in zoom-in duration-300">
    <div class="bg-white/95 backdrop-blur-2xl rounded-2xl p-7 sm:p-9 w-full max-w-sm shadow-2xl border border-white/90 relative flex flex-col items-center text-center gap-5">
        ...
        {{-- Khung hiển thị QR --}}
        <div class="relative w-52 h-52 bg-white rounded-2xl border border-slate-200/80 p-3 shadow-sm group/qr overflow-hidden flex items-center justify-center">
            <img id="qrModalImage" src="" alt="QR Code" class="w-full h-full object-contain rounded-xl">
            {{-- Hover Save Overlay --}}
            <button onclick="LinkManager.saveQR()" class="absolute inset-0 bg-indigo-600/85 opacity-0 group-hover/qr:opacity-100 transition-opacity flex items-center justify-center text-white flex-col gap-2 rounded-2xl">
        ...
        <div class="w-full flex items-center gap-2.5">
            <button onclick="LinkManager.shareLink()" class="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white font-bold font-outfit py-2.5 rounded-xl transition-all shadow-sm uppercase tracking-wider text-xs flex items-center justify-center gap-1.5 active:scale-95">
            ...
            <button onclick="LinkManager.saveQR()" class="flex-1 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold font-outfit py-2.5 rounded-xl transition-all uppercase tracking-wider text-xs flex items-center justify-center gap-1.5 active:scale-95">
