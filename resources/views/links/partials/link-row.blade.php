...
<tr class="hover:bg-indigo-50/20 transition-colors group">
    ...
    <td class="px-5 sm:px-6 py-4 text-center">
        <span class="inline-flex items-center gap-1.5 px-3 py-1 bg-indigo-50 text-indigo-700 rounded-xl font-bold font-outfit text-xs border border-indigo-100/80">
    ...
    <td class="px-5 sm:px-6 py-4">
        <div class="flex items-center justify-end gap-1.5">
            <button onclick="LinkManager.toggleStatus('{{ $link->short_code }}', this)" 
                class="p-2 rounded-xl transition-all shadow-sm active:scale-90 border {{ $link->is_active ? 'bg-emerald-50 text-emerald-600 hover:bg-emerald-600 hover:text-white border-emerald-100' : 'bg-rose-50 text-rose-500 hover:bg-rose-500 hover:text-white border-rose-100' }}" 
            ...
            <a href="{{ route('links.show', $link->short_code) }}" class="p-2 rounded-xl bg-slate-50 text-slate-500 hover:bg-indigo-50 hover:text-indigo-600 border border-slate-200/60 transition-all shadow-sm active:scale-90" title="Xem thống kê">
            ...
            <button onclick="LinkManager.showQR('{{ url($link->short_code) }}')" class="p-2 rounded-xl bg-slate-50 text-slate-500 hover:bg-indigo-50 hover:text-indigo-600 border border-slate-200/60 transition-all shadow-sm active:scale-90" title="Mã QR">
            ...
            <button onclick="LinkManager.deleteLink('{{ $link->short_code }}')" class="p-2 rounded-xl bg-rose-50 text-rose-500 hover:bg-rose-500 hover:text-white border border-rose-100 transition-all shadow-sm active:scale-90" title="Xóa liên kết">
